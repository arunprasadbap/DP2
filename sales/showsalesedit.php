...
<script type="text/javascript">
  ...
  function updateRow(salesID,itemID){

    var quantityVal=$("#quantity"+salesID).val();
    var invVal=$("#inv"+salesID).val();
    var dateVal=$("#date"+salesID).val();
    ...
    $.ajax({
      method: "POST",
      url: "editsale.php",
      data: { salesId: salesID, itemId: itemID, quantity:quantityVal, invoicenumber:invVal, date:dateVal  }
    })
      .done(function( msg ) {
        quantity.html(quantityVal);
        inv.html(invVal);
        date.html(dateVal);

        price.html(price.attr('price')*quantityVal);
        btn.html('<button onClick="return editRow('+salesID+','+itemID+')" type="button">Edit</button>');
      });
